...
class TightMazeDQNTrainer(Node):
    ...
    def select_action(self, state):
        ...
        front_clearance = state[-1] * 1.0  # Denormalize front clearance
        
        # Get actual minimum distance for better decision making
        min_laser = self.get_min_laser_distance()
        
        # FORCED EXPLORATION in tight spaces
        if random.random() < self.epsilon:
            # Smart random: if VERY close to obstacle, MUST rotate
            if min_laser < 0.22:
                return random.choice([0, 2])  # Only rotate
            # If somewhat close, prefer rotation 70% of time
            elif min_laser < 0.35:
                if random.random() < 0.7:
                    return random.choice([0, 2])
                return 1  # Sometimes try forward
            # If clear, any action
    ...
    def calculate_reward(self, state, next_state, done, action):
        ...
        # COLLISION - penalty decreases as agent learns
        if min_laser < 0.17:
            self.collision_count += 1
            # Adaptive penalty: harsh early, softer later
            penalty = -50.0 if self.epsilon > 0.7 else -30.0
            return penalty
        ...
        # 2. PROGRESS TOWARD GOAL (MOST IMPORTANT)
        distance_delta = self.previous_distance - current_dist
        if distance_delta > 0.001:  # Moved closer (with small threshold)
            # MASSIVE reward for progress
            reward += distance_delta * 300.0  # Increased from 200.0
            ...
            if current_dist < self.best_distance:
                ...
                reward += improvement * 100.0  # Big bonus
                self.get_logger().info(f'🏆 New record: {current_dist:.3f}m (Δ{improvement:.3f}m)')
        elif distance_delta < -0.001:  # Moved away
            reward += distance_delta * 50.0  # Mild penalty
        
        # 3. DISTANCE-BASED PRESSURE
        # Constant pressure to be closer to goal
        reward -= current_dist * 2.0
        
        # 4. CLEARANCE MANAGEMENT
        if min_laser < 0.20:
            reward -= 15.0  # Danger zone
        elif min_laser < 0.25:
            reward -= 5.0   # Close call
        elif min_laser > 0.35:
            reward += 1.0   # Safe distance bonus
        
        # 5. HEADING REWARD (only when safe)
        if min_laser > 0.28:
            goal_angle_error = abs(next_state[self.laser_bins + 1])
            # Reward for facing goal
            heading_reward = (1.0 - goal_angle_error) * 3.0
            reward += heading_reward
        
        # 6. FORWARD PROGRESS BONUS
        if action == 1 and min_laser > 0.30:  # Forward action when safe
            reward += 2.0
        
        # 7. MILESTONE REWARDS (these work!)
        if self.step == 20:
            reward += 20.0
            self.get_logger().info('💪 20 steps!')
        elif self.step == 50:
            reward += 40.0
    ...
    def check_done(self):
        ...
        min_laser = self.get_min_laser_distance()
        if min_laser < 0.17:
            # Count survival before returning
            if self.step >= 20:
                self.survival_count += 1
            return True
            
        if self.step >= self.max_steps_per_episode:
            # Count survival for timeout too
            self.survival_count += 1
            return True
    ...
    def train(self, num_episodes=3000):
        ...
        for episode in range(num_episodes):
            ...
            avg_steps = np.mean([ep for ep in self.episode_rewards[-50:] if ep != 0]) if len([ep for ep in self.episode_rewards[-50:] if ep != 0]) > 0 else self.step
            
            # Progress indicator
            progress_pct = (1.0 - self.best_distance / 0.962) * 100  # 0.962 = initial distance
            
            self.get_logger().info(
                f'📊 Ep {episode} | Steps: {self.step} | R: {self.episode_reward:.1f} | '
                f'Avg: {avg_reward:.1f} | ✓{self.success_count} ✗{self.collision_count} '
                f'💪{survival_rate:.0f}% | Best: {self.best_distance:.3f}m ({progress_pct:.1f}%) | ε: {self.epsilon:.2f}'
            )
